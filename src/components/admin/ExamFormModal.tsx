...
import { X, Search, CheckSquare, Square, Wand2, List, RefreshCw, Loader2 } from "lucide-react";
...
const DIFF_MIXES: { id: DiffMix; label: string; desc: string; color: string }[] = [
  { id: "progressive", label: "Progressive",  desc: "20% Easy · 60% Medium · 20% Hard", color: "amber"   },
  { id: "balanced",    label: "Balanced",      desc: "33% Easy · 34% Medium · 33% Hard", color: "blue"    },
  { id: "challenge",   label: "Challenge",     desc: "10% Easy · 40% Medium · 50% Hard", color: "red"     },
];
...
export function ExamFormModal({ open, onClose, categories, questions, exam }: Props) {
  ...
  // ── Availability map per category per difficulty (filtered by licenseCategory) ──
  const availMap = useMemo(() => {
    const source = licenseCategory
      ? questions.filter((q) => q.license_categories.includes(licenseCategory))
      : questions;
    ...
    return m;
  }, [questions, licenseCategory]);

  // ── Exam metadata category: filter by license type ──
  const visibleCategories = useMemo(() => {
    if (!licenseCategory) return categories;
    return categories.filter(
      (c) => !c.license_category || c.license_category === licenseCategory
    );
  }, [categories, licenseCategory]);

  // ── Smart Build: only show categories scoped to the selected license type ──
  ...
  // Reset when license category changes — clear stale category, rows, and question selections
  useEffect(() => {
    setCategoryId("");
    setCatRows((prev) => prev.map((r) => ({ ...r, enabled: false, count: 0 })));
    setBreakdown(null);
    setSelectedIds([]);
  }, [licenseCategory]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (exam) {
      ...
      setQMode("manual");   // show existing set when editing
    } else {
      setTitleEn("");
      setDescription("");
  ...
  const filteredQuestions = useMemo(() => {
    let qs = licenseCategory
      ? questions.filter((q) => q.license_categories.includes(licenseCategory))
      : questions;
    const s = qSearch.toLowerCase();
    return s
      ? qs.filter((q) => q.text_en.toLowerCase().includes(s) || q.category.name_en.toLowerCase().includes(s))
      : qs;
  }, [questions, qSearch, licenseCategory]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-3xl max-h-[92vh] bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          {/* Title */}
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Exam Title *</label>
            <input
              value={titleEn}
              onChange={(e) => setTitleEn(e.target.value)}
              placeholder="e.g. Internal Medicine — Mock Exam"
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">
              ...
              <span className="font-normal ml-1 text-gray-400">(restricts visibility to this license group)</span>
            </label>
            <select
              value={licenseCategory}
              onChange={(e) => { setLicenseCategory(e.target.value); setBreakdown(null); }}
            ...
            {licenseCategory && (
              <p className="mt-1.5 text-xs text-purple-600 dark:text-purple-400">
                Only students with the &quot;{LICENSE_CATEGORIES.find((c) => c.id === licenseCategory)?.label}&quot; license category will see this exam.
                Smart Build and Manual mode will only show questions tagged for this category.
              </p>
            )}
          </div>
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5">Passing Score (%)</label>
          ...
          <div>
            ...
            <div className="flex items-center justify-between mb-3">
              <label className="text-xs font-semibold text-gray-500 dark:text-gray-400">
                Questions * <span className="font-normal text-gray-400">({selectedIds.length} selected)</span>
              </label>
              <div className="flex items-center gap-1 p-1 bg-gray-100 dark:bg-gray-800 rounded-lg">
                <button
                  type="button"
                  onClick={() => setQMode("smart")}
                ...
                >
                  <List className="w-3.5 h-3.5" /> Manual
                </button>
              </div>
            </div>
            ...
            {qMode === "smart" && (
              <div className="rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                {/* Category rows — only show categories that have questions available */}
                <div className="divide-y divide-gray-100 dark:divide-gray-800">
                  ...
                  {visibleCatRows.filter((row) => (availMap[row.catId]?.total ?? 0) > 0).length === 0 && (
                    <div className="px-4 py-6 text-center text-sm text-gray-400">
                      {licenseCategory
                        ? "No questions are tagged for this license category yet."
                        : "No approved questions in the question bank yet."}
                    </div>
                  )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 w-32 justify-end">
                          <span className="text-[10px] text-gray-400">{avail.total} total</span>
                        ...
                        <div key={b.category_id} className="px-4 py-2.5 flex items-center justify-between">
                          <div>
                            ...
                            {b.selected < b.requested && (
                              <p className="text-[10px] text-amber-600 dark:text-amber-400">Only {b.selected} of {b.requested} available</p>
                            )}
                          </div>
              ...
              <>
                ...
                <div className="max-h-60 overflow-y-auto rounded-xl border border-gray-200 dark:border-gray-800 divide-y divide-gray-100 dark:divide-gray-800">
                  {filteredQuestions.length === 0 && (
                    <p className="text-center text-sm text-gray-400 py-6">No questions found</p>
                  )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-gray-700 dark:text-gray-300 line-clamp-1">{q.text_en}</p>
                        </div>
        ...
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-gray-200 dark:border-gray-800 flex-shrink-0 bg-gray-50 dark:bg-gray-900">
          <p className="text-xs text-gray-400">{selectedIds.length} question{selectedIds.length !== 1 ? "s" : ""} selected</p>
