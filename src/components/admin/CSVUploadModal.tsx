...
export function CSVUploadModal({ open, onClose }: CSVUploadModalProps) {
  ...
  async function handleUpload() {
    if (!csvText.trim()) return toast.error("No CSV content to upload");
    setUploading(true);
    setResult(null);
    try {
      ...
      <div className="relative w-full max-w-xl bg-white dark:bg-gray-900 rounded-2xl shadow-2xl flex flex-col overflow-hidden max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-800">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">Upload a CSV file to add multiple questions at once</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          ...
          <div className="rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
            <div className="px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-800">
              <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Required CSV Columns</p>
            </div>
          ...
          {!fileName && (
            <div>
              ...
              <textarea
                ...
                rows={4}
                placeholder="category_slug,difficulty,text_en,..."
          ...
          {result && (
            <div className="space-y-2">
              <div className="flex items-center gap-2 p-3 rounded-xl bg-emerald-50 dark:bg-emerald-900/10 border border-emerald-100 dark:border-emerald-800/30">
                ...
                <p className="text-sm font-medium text-emerald-700 dark:text-emerald-400">{result.imported} question{result.imported !== 1 ? "s" : ""} imported successfully</p>
              </div>
              {result.failed.length > 0 && (
                <div className="p-3 rounded-xl bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-800/30 space-y-1">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm font-medium text-red-700 dark:text-red-400">{result.failed.length} row{result.failed.length > 1 ? "s" : ""} failed</p>
                  </div>
