...
export async function POST(req: NextRequest) {
  ...
  const { text_en, text_fr, explanation_en, explanation_fr, difficulty, category_id, answers, license_categories, language = "EN" } = body;

  const primaryText = language === "FR" ? text_fr : text_en;
  if (!primaryText || !category_id || !Array.isArray(answers) || answers.length < 2)
